...
DESCRIPTIONS: Dict[str, str] = {
    ...
    "getLoreBooks": "Find local, character-global, and enabled-module lorebook entries whose `comment` exactly matches `search`; returned `content` is CBS-parsed.",
    "loadLoreBooks": "Load activated lorebooks and JSON-decode them (low-level, no reserve argument).",
    "LLM": "Run a sub-request against the main model (low-level); roles accept `system`/`sys`, `user`, `assistant`/`bot`/`char`; optional multimodal extraction handles inlay tokens.",
    ...
    # host functions
    "getChatVar": "Read a chat variable (string; falls back to character/template default variables, then `\"null\"`).",
    ...
    "setChat": "Replace the text of the message at an index; negative indexes work like JS `Array.at`; out-of-range indexes no-op.",
    "setChatRole": "Set role to `user`; any other value becomes `char`; negative indexes work like JS `Array.at`; out-of-range indexes no-op.",
    ...
    "getCharacterLastMessage": "Most recent character-role message, or the character's first message if none exists.",
    "getUserLastMessage": "Most recent user-role message, or an empty string if none exists.",
